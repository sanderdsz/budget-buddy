...
export default function Profile() {
  ...
  const fetchUser = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`/users/email?email=${userConnectionEmail}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  const handleNewUserConnection = async () => {
    ...
    try {
      const response = await api.post(`/users/connections/new`, connectionData, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
              ...
              <div className={`${styles[`profile__container--small`]}`}>
                <div className={`${styles[`profile-connection__container`]}`}>
                  ...
                  {isLoading ? (
                    ...
                    <div className={`${styles[`profile__container--connection`]}`}>
                      {userConnection ? (
                        <div className={`${styles[`profile-connection__list`]}`}>
                          <span>{userConnection.firstName}{" "}{userConnection.lastName}</span>
                          <Button
                            label={"connect"}
                            colour={"success"}
