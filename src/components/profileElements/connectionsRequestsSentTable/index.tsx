...
export default function ConnectionsRequestsSentTable() {
	...
	return (
		<div>
			<span className={`${styles[`connected-users__title`]}`}>Connections</span>
								...
								<tbody>
									{userConnections.map((user, index) => (
										<tr key={index} className={`${styles[``]}`}>
											...
											<td className={`${styles[`user-children__email`]}`}>
												{user.email}
											</td>
											{
												// @ts-ignore
